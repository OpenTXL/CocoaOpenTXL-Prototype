...
@interface TXLRingTest : GHTestCase {
    
}

@end

@implementation TXLRingTest
...
- (void)testCreateRing {
    ...
    TXLRing *ring = [[TXLRing alloc] initWithPoints:points
                                          clockwise:YES];
    
    GHAssertNotNil(ring, nil);
    
    GHAssertTrue(ring.clockwise, nil);
    
    // check bbox
    GHAssertEquals(ring.boundingBox.minLatitude, 15.0, nil);
    GHAssertEquals(ring.boundingBox.maxLatitude, 17.0, nil);
    GHAssertEquals(ring.boundingBox.minLongitude, 10.0, nil);
    GHAssertEquals(ring.boundingBox.maxLongitude, 15.0, nil);
    
    // coords
    __block int i = 0;
    [ring iterateOverPoints:^(TXLCoordinate coordinate, BOOL *stop){
